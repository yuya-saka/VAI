...
def validate_homogeneous_gpus(gpu_ids: list[int]) -> dict[str, object]:
    """GPUのmodelとcompute capabilityが一致することを検証する。"""
...
def launch_fold_processes(
    ...
    *,
    ...
    """固定GPU割当でfold子processを最大指定数まで並列実行する。"""
    ...
    _write_execution_plan(experiment_root, mapping, gpu_signature)
    ...
    while pending or running:
        ...
        while pending and len(running) < concurrency:
            ...
            command = _build_worker_command(
                config_path, module_name, outer_fold, gpu_id, resume
            )
            print(
                f"[parallel] outer={outer_fold}をGPU {gpu_id}で起動します",
                flush=True,
            )
...
def _write_execution_plan(
    ...
    """再現用GPU割当を保存し、異なる割当での上書きを拒否する。"""
    path = experiment_root / "fold_execution_plan.json"
    payload = {
        "mode": "fold",
