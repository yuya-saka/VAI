...
> **2026-07-19更新**: 本文のhardな「bbox overlapあり=region positive」方式は廃止する。
> bboxは軸補正後planeへ物理座標でsoft evidenceとして再サンプリングし、
> confidence-weightedな条件付き4領域分布として使う。
> 現行方針は [`bbox_4領域ソフトラベル設計.md`](bbox_4領域ソフトラベル設計.md) を参照。
