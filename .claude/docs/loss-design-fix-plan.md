...
**Implementation Order (Codex Verified):** #3 → #2 → #1
- #3 (NaN handling) は必須前提条件
- #2 (smoothing removal) で rho_loss スケールを確定
- #1 (warmup formula) は全体バランス調整（#2 後に適用）

---

#### 1.3 Fix NaN Handling Order **← FIRST (最優先)**
...
**Codex Verified Order: #3 → #2 → #1**

### Phase 1: Critical Fixes (訓練失敗の原因)
- [ ] **1.3 FIRST: Fix NaN handling order** (line_losses.py:191-278) ← 最優先
- [ ] **1.2 SECOND: Remove double smoothing** (line_losses.py:265)
- [ ] **1.1 THIRD: Warmup formula fix** (train_heat.py:840)
- [ ] Unit tests実行
- [ ] Integration test実行 (10 epochs)

### Phase 2: Loss Balance (重要)
- [ ] 2.1: Increase lambda_rho (config.yaml)
- [ ] 2.2: Disable scheduler during warmup (train_heat.py:863)
- [ ] Full validation (1 fold)

### Phase 3: Tuning (オプション)
- [ ] Phase 3調整（必要に応じて）
