...
```bash
# Analysis (read-only)
codex exec --model gpt-5.6-sol --sandbox read-only --full-auto "{question}" 2>/dev/null

# Implementation work (can write files)
codex exec --model gpt-terra --sandbox workspace-write --full-auto "{task}" 2>/dev/null
```
