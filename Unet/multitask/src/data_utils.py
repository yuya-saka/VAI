...
def _make_weighted_sampler(
    dataset: PngLineDataset, seg_labeled_weight: float = 3.0
) -> WeightedRandomSampler:
    '''seg GTありサンプルを高頻度サンプリングするWeightedRandomSamplerを作成する

    引数:
        dataset: PngLineDataset インスタンス
...
def create_model_optimizer_scheduler(cfg, device):
    ...
    line_channels = int(model_cfg.get('line_channels', 4))
    ...
    decoder_type = str(model_cfg.get('decoder_type', 'dual_decoder'))
