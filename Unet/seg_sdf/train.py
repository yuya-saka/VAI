...
from seg_sdf.src.data_utils import load_config, set_seed
from seg_sdf.src.trainer import train_one_fold


def parse_args() -> argparse.Namespace:
    """CLI引数を解析する。"""
    parser = argparse.ArgumentParser(description="5-Fold CV 全自動実行（Seg+SDF）")
...
def main() -> None:
    ...
    print("\n[Auxiliary Metrics - SDF]")
    if "test_sdf_loss" in avg:
        print(f"  Test SDF Loss:         {avg['test_sdf_loss']:.6f}")
    if "test_seg_miou" in avg:
        print(f"  Test Seg mIoU:         {avg['test_seg_miou']:.4f}")
    if "test_sdf_mae" in avg:
        print(f"  Test SDF MAE:          {avg['test_sdf_mae']:.4f}")
    if "test_sdf_boundary_iou" in avg:
        print(f"  Test SDF Boundary IoU: {avg['test_sdf_boundary_iou']:.4f}")
