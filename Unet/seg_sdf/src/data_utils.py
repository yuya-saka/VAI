...
def prepare_datasets_and_splits(cfg):
    ...
    return train_s, val_s, test_s, root_dir, group, image_size, sdf_tau, seed
...
def _make_weighted_sampler(
    dataset: PngSdfDataset, seg_labeled_weight: float = 3.0
) -> WeightedRandomSampler:
    '''seg GTありサンプルを高頻度サンプリングするWeightedRandomSamplerを作成する

    引数:
        dataset: PngSdfDataset インスタンス
        seg_labeled_weight: seg GT あり時のサンプリング重み（デフォルト 3.0）

    戻り値:
        WeightedRandomSampler
    '''
    weights = [
        seg_labeled_weight if item.get('gt_mask_path') is not None else 1.0
...
def create_data_loaders(
    ...
    val_ds = PngSdfDataset(
        root_dir, val_samples, group=group, image_size=image_size, sdf_tau=sdf_tau
    )
    test_ds = PngSdfDataset(
        root_dir, test_samples, group=group, image_size=image_size, sdf_tau=sdf_tau
    )
...
def create_model_optimizer_scheduler(cfg, device):
    ...
    sdf_channels = int(model_cfg.get('sdf_channels', 4))
